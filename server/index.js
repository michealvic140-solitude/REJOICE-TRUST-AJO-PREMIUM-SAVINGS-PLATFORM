...
app.delete("/api/admin/groups/:id", adminRequired, async (req, res) => {
  try {
    const groupId = req.params.id;
    const { rows } = await db(`SELECT name FROM groups WHERE id=$1`, [groupId]);
    if (!rows[0]) return res.status(404).json({ error: "Group not found" });
    await db(`DELETE FROM groups WHERE id=$1`, [groupId]);
    await addAuditLog(`Admin deleted group: ${rows[0].name}`, req.session.userId, req.session.username, "delete");
    res.json({ success: true });
...
app.get("/api/admin/defaulters", adminRequired, async (req, res) => {
  ...
  res.json(rows.map(d => ({ id: d.id, user: d.username, userId: d.user_id, group: d.group_name, groupId: d.group_id, seatNo: d.seat_no, since: d.missed_since || d.created_at, count: d.missed_count || 1, amount: `₦${Number(d.amount || d.total_amount || 0).toLocaleString()}` })));
});
