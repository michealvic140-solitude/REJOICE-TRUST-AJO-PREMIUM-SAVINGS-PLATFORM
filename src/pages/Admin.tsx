...
import {
  ...
  TrendingUp, LogOut, Trash2, Phone, Key, Mail,
...
type SideTab =
  ...
  | "support" | "contact-info" | "group-messages" | "seat-removal";
...
const SIDEBAR_ITEMS: { id: SideTab; icon: React.FC<{ size?: number }>; label: string; adminOnly?: boolean }[] = [
  ...
  { id: "seat-removal",   icon: Trash2,        label: "Seat Removals",   adminOnly: true },
...
export default function Admin() {
  ...
  const annVideoRef                           = useRef<HTMLInputElement>(null);
  ...
  const loadData = async (tab: SideTab) => {
    ...
    try {
      switch (tab) {
        ...
        case "seat-removal":  setSeatRemovals(await api.get("/api/admin/seat-removal")); break;
  ...
  const deleteGroup = async (groupId: string, groupName: string) => {
    if (!confirm(`Delete group "${groupName}"? This cannot be undone. All slots, payments, and chat history will be removed.`)) return;
    try {
  ...
  const lockSeat = async (groupId: string) => {
    const seatNo = parseInt(prompt("Enter seat # to reserve (lock):") || "0");
    if (!seatNo) return;
    await doGroupAction(groupId, "lockSeat", { seatNo, lock: true });
  };

  const unlockSeat = async (groupId: string) => {
    const seatNo = parseInt(prompt("Enter seat # to unreserve (unlock):") || "0");
    if (!seatNo) return;
    await doGroupAction(groupId, "lockSeat", { seatNo, lock: false });
  };
  ...
  const doSeatRemoval = async (id: string, status: string) => {
    try {
      await api.patch(`/api/admin/seat-removal/${id}`, { status });
      setSeatRemovals(prev => prev.map((r: Record<string, unknown>) => r.id === id ? { ...r, status } : r));
    } catch (e) { alert((e as Error).message); }
  };

  const setTrustScore = async (userId: string) => {
    const score = prompt("Enter trust score (0–100):");
    if (score === null) return;
    const num = parseInt(score);
    if (isNaN(num) || num < 0 || num > 100) return alert("Please enter a number between 0 and 100");
    try {
      await api.patch(`/api/admin/users/${userId}`, { action: "trustScore", trustScore: num });
      const updated = await api.get("/api/admin/users");
      setAdminUsers(updated);
    } catch (e) { alert((e as Error).message); }
  };
  ...
  return (
    <div className="min-h-screen pt-16 flex relative">
      ...
      <main className="ml-52 flex-1 p-6 relative z-10">
                ...
                <TR key={u.id as string}>
                  ...
                  <TD>
                    <div className="flex flex-wrap gap-1">
                      ...
                      <Btn size="xs" variant="amber" onClick={() => setTrustScore(u.id as string)}><Star size={10} />Trust Score: {u.trustScore ?? 50}</Btn>
                    </div>
                  </TD>
                </TR>
              ))}
                ...
                <TR key={g.id}>
                  ...
                  <TD>
                    <div className="flex flex-wrap gap-1">
                      ...
                      <Btn size="xs" variant="amber" onClick={() => lockSeat(g.id)}>
                        <Lock size={10} />Reserve Seat
                      </Btn>
                      <Btn size="xs" variant="glass" onClick={() => unlockSeat(g.id)}>
                        <Lock size={10} />Unreserve
                      </Btn>
                      <Btn size="xs" variant="red" onClick={() => deleteGroup(g.id, g.name)}>
        ...
        {/* ══ SEAT REMOVALS ════════════════════════════════════════════════════ */}
        {sideTab === "seat-removal" && isAdmin && (
          <div>
            <SectionHeader title="Seat Removal Requests" sub={`${seatRemovals.length} total requests`}
              actions={<Btn onClick={() => loadData("seat-removal")} variant="glass"><RefreshCw size={12} /></Btn>} />
            <Table cols={["User", "Group", "Seat #", "Reason", "Status", "Date", "Actions"]}>
              {seatRemovals.map((r: Record<string, unknown>) => (
                <TR key={r.id as string}>
                  ...
                  <TD><span className="text-muted-foreground">{r.group_name as string}</span></TD>
                  <TD><span className="text-amber-400 font-bold">#{r.seat_number as number}</span></TD>
                  ...
                  <TD><span className="text-muted-foreground">{new Date(r.requested_at as string).toLocaleDateString()}</span></TD>
                  <TD>
        ...
        <Modal title="Post Announcement" onClose={() => setShowAnnouncement(false)}>
          <div className="space-y-4">
            ...
            <div>
              ...
              <div onClick={() => annImageRef.current?.click()} className="flex items-center gap-3 p-3 rounded-xl border border-dashed border-white/20 bg-white/3 cursor-pointer hover:border-gold/30 hover:bg-gold/5 transition-all">
                <span className="text-muted-foreground text-xs flex-1">{annImage ? `✅ ${annImage.name}` : "Click to attach image"}</span>
                {annImage && <button type="button" onClick={e => { e.stopPropagation(); setAnnImage(null); }} className="text-red-400 text-xs">Remove</button>}
              </div>
              <input ref={annImageRef} type="file" accept="image/*" className="hidden" onChange={e => setAnnImage(e.target.files?.[0] || null)} />
            </div>
            <div>
              <label className="luxury-label">Video (optional)</label>
              <div onClick={() => annVideoRef.current?.click()} className="flex items-center gap-3 p-3 rounded-xl border border-dashed border-white/20 bg-white/3 cursor-pointer hover:border-gold/30 hover:bg-gold/5 transition-all">
                <span className="text-muted-foreground text-xs flex-1">{annVideo ? `✅ ${annVideo.name}` : "Click to attach video"}</span>
                {annVideo && <button type="button" onClick={e => { e.stopPropagation(); setAnnVideo(null); }} className="text-red-400 text-xs">Remove</button>}
              </div>
              <input ref={annVideoRef} type="file" accept="video/*" className="hidden" onChange={e => setAnnVideo(e.target.files?.[0] || null)} />
            </div>
