...
import {
  ...
  ChevronDown, Bell, Clock, CreditCard, AlertTriangle, Trash2
} from "lucide-react";
import ParticleBackground from "@/components/ParticleBackground";
import SeatRemovalModal from "@/components/SeatRemovalModal";
...
export default function GroupDetail() {
  ...
  const [showSeatRemoval, setShowSeatRemoval] = useState(false);
  ...
  return (
    <div className="min-h-screen pt-16 pb-16 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
          ...
          <div className="xl:col-span-2 space-y-4">
            ...
            <div className="glass-card-static rounded-2xl p-4 animate-fade-up delay-100">
              ...
              <div className="mt-3 pt-3 border-t border-gold/10 flex flex-wrap items-center gap-4 text-xs">
                ...
                {mySeats.length > 0 && (
                  <div className="ml-auto flex flex-wrap gap-1.5">
                    {mySeats.map(s => (
                      <button
                        key={s.id}
                        onClick={() => { setSeatRemovalSeatNo(s.id); setShowSeatRemoval(true); }}
                        className="flex items-center gap-1 px-2 py-1 rounded-lg border border-red-600/30 bg-red-900/15 text-red-400 hover:bg-red-900/25 transition-all text-[10px] font-semibold"
                      >
                        <Trash2 size={9} /> Remove Seat #{s.id}
                      </button>
                    ))}
                  </div>
                )}
      ...
      {/* ═══ SEAT REMOVAL MODAL ═══════════════════════════════════════════════ */}
      {showSeatRemoval && seatRemovalSeatNo !== null && (
        <SeatRemovalModal
          groupId={Number(id)}
          groupName={group.name}
          seatNumber={seatRemovalSeatNo}
          onClose={() => { setShowSeatRemoval(false); setSeatRemovalSeatNo(null); }}
          onSuccess={() => loadSlots()}
        />
      )}
    </div>
